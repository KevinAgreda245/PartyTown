...
if (isset($_SESSION['idUsuario'])) {
    ...
    $pdf->head('Reporte de Producto por Tipo de Producto');
    $pdf->SetWidths(Array(50,80,30,30));
    $pdf->SetLineHeight(5);
    $pdf->SetFont('Times','',12);
    $typeProd = ' ';
    $dat = $type->getTypeProd();

    foreach($dat as $index){
        if(utf8_decode($index['tipoProducto']) != $type){
            $pdf->Ln(5);
            $pdf->SetTextColor(255,255,255); 
            $pdf->SetFillColor(28, 185, 158); 
            $pdf->SetFont('Times','B',12);
            $pdf->Cell(190,10,utf8_decode($index['tipoProducto']),0,0,'L',true);
            $pdf->Ln(15);
            ...
            $pdf->SetFont('Times','',12);
            $type = utf8_decode($index['tipoProducto']);
        }
        $pdf->Row(Array(
