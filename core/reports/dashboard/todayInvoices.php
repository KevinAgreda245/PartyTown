...
if ($_SESSION['idUsuario']) {
    ...
    $pdf->head('Reporte de Facturas Diarias');
    $pdf->SetFont('Times','',12);
    $id = '';
    $total = 0;
    $data = [];
    $invoices->setFecha(date('Y-m-d'));
    $data = $invoices->getToday();
    if($data){
        foreach($data as $index){
            ...
            $pdf->SetFont('Times','B',12);
            $pdf->Cell(18,10,utf8_decode('Total($):'),0,0,'L',true);
            $pdf->SetFont('Times','',12);
            $pdf->Cell(170,10,utf8_decode(number_format($total,2,".",",")),0,1,'L',true);
            $pdf->SetFont('Times','B',12);
            $pdf->Cell(16,10,utf8_decode('Estado:'),0,0,'L',true);
            $pdf->SetFont('Times','',12);
            if($index['estadoFactura'] == 0){
                $pdf->Cell(169,10,utf8_decode('Pendiente'),0,1,'L',true);
            } else {
                $pdf->Cell(169,10,utf8_decode('Entregada'),0,1,'L',true);
            }
            $pdf->Ln(5);
        ...
        $pdf->SetFont('Times','B',15);
        $pdf->Cell(190,10,utf8_decode('No hay facturas generadas este día'),0,0,'C');
    }
    $pdf->Output();
} else {
